EmployeesController = MeteorisController.extend({
    /* get subscribtion from server with parameter criteria, and sort/limit */
    subscriptions: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();

        this.subscription = this.subs.subscribe('employees', this.getCriteria(), sort);
    },
...
{birthdate: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Employees.find(this.getCriteria(), sort);

        return this.render('employeesIndex', {
            data: {
    ...
    view: function() {
        return this.render('employeesView', {
            data: {
...
jobid: t.find('#jobid').value,
birthdate: t.find('#birthdate').value? new Date(t.find('#birthdate').value):null,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Employees.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Employees");
                Router.go('employeesView', {_id: _id});
            });
        }
        return this.render('employeesInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Employees.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Employees");
            });
            Router.go('employeesView', {_id: _id});
        }
        return this.render('employeesUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Employees.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Employees");
        });
    },
